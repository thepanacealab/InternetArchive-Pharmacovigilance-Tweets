...
fO = open("predictions_svm_all.txt", "w")
...
for source in df['source'].unique():
    ...
    #print("Mean AUC Score - SVM: ", rfc_cv_score.mean())
    #print('Accuracy for {} data: {:.4f}'.format(source, score))

    print("starting predictions")
    #predictions = classifier.predict(X_predict)
    probabilities = clf.predict_proba(X_predict)[:,1]
    
    for i in range(len(probabilities)):
    
        #fO.write( str(predictions[i]) + "\n")
        fO.write( predict_sentences[i]+ "\t" + tweetIds[i] + "\t" + userIds[i] + "\t" +  str(probabilities[i]) + "\n")
